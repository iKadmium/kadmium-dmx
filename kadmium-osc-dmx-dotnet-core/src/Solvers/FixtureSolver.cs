using kadmium_osc_dmx_dotnet_core.Fixtures;
using Newtonsoft.Json.Linq;
using System.Collections.Generic;
using System.Linq;

namespace kadmium_osc_dmx_dotnet_core.Solvers
{
    public abstract class FixtureSolver : Solver
    {
        public FixtureSolver(Fixture fixture, params string[] attributes) : base(fixture.Settables, fixture.FrameSettables, from attributeName in attributes select new FixtureSolverAttribute(attributeName))
        { }

        public static IEnumerable<FixtureSolver> GetDefaultSolvers(Fixture fixture, JObject options)
        {
            List<FixtureSolver> solvers = new List<FixtureSolver>();
            if (MasterAtFullSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new MasterAtFullSolver(fixture));
            }
            if (FakeUVSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new FakeUVSolver(fixture));
            }
            if (FakeStrobeSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new FakeStrobeSolver(fixture));
            }
            if (ApeshitFixtureSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new ApeshitFixtureSolver(fixture));
            }
            if (HSBtoRGBWSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new HSBtoRGBWSolver(fixture));
            }
            if (HSBtoRGBSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new HSBtoRGBSolver(fixture));
            }
            if (HSBtoColorWheelSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new HSBtoColorWheelSolver(fixture));
            }
            if (BrightnessLimiterSolver.SuitableFor(fixture.FixtureDefinition, options))
            {
                solvers.Add(new BrightnessLimiterSolver(fixture, options));
            }
            if (RandomMove.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new RandomMove(fixture));
            }
            if (MovementRestrictionSolver.SuitableFor(fixture.FixtureDefinition, options))
            {
                solvers.Add(new MovementRestrictionSolver(fixture, options));
            }
            if (MovementInversionSolver.SuitableFor(fixture.FixtureDefinition, options))
            {
                solvers.Add(new MovementInversionSolver(fixture, options));
            }
            if (PanTilt16BitSolver.SuitableFor(fixture.FixtureDefinition))
            {
                solvers.Add(new PanTilt16BitSolver(fixture));
            }
            return solvers;
        }
    }
}

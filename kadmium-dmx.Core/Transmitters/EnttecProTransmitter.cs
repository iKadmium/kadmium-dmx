...
namespace kadmium_dmx_core.Transmitters
{
    ...
    {
        private static readonly int DMX_PRO_MIN_DATA_SIZE = 25;
        private static readonly byte DMX_PRO_MESSAGE_START = 0x7E;
        private static readonly byte DMX_PRO_MESSAGE_END = 0xE7;
        private static readonly byte DMX_PRO_SEND_PACKET = 6;

        public string Port { get; set; }

        public SerialPortStream Stream { get; set; }

        public EnttecProTransmitter(IEnttecProTransmitterSettings data) : base(data.Delay)
        {
            Delay = data.Delay;
            Port = data.Port;
            TryOpenStream();
